    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="grid grid-cols-1 gap-8">
        {models.map((model) => {
          ...
          return (
            <div key={model.id} className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-colors duration-300">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-gray-100 dark:border-slate-700 pb-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-12 bg-gray-50 dark:bg-slate-700 rounded flex items-center justify-center p-1">
                    <img src={model.heroImage} alt={model.name} className="w-full h-full object-contain" />
                  </div>
                  <div>
                    <h4 className="text-lg font-bold font-display text-slate-900 dark:text-white">{model.name}</h4>
                  ...
                  <div className="flex items-center">
                    {isConfirming ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 text-xs font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-900/40 px-3 py-1.5 rounded-lg border border-orange-200 dark:border-orange-800 transition-colors disabled:opacity-50"
              ...
              <div className="space-y-4">
                ...
                <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-2 bg-gray-50 dark:bg-slate-700/50 rounded-lg text-xs font-bold uppercase text-gray-500 dark:text-gray-400">
                ...
                {model.variants.map(variant => {
                  ...
                  return (
                    <div key={variant.id} className={`flex flex-col md:grid md:grid-cols-12 gap-4 items-center px-4 py-4 rounded-lg border transition-all ${variant.soldOut
                      ...
                      <div className="w-full md:col-span-3 flex justify-between items-center md:block">
                        <span className="md:hidden text-xs font-bold uppercase text-gray-400 dark:text-gray-500">Variant</span>
                        <span className={`font-bold text-sm ${variant.soldOut ? 'text-gray-500 dark:text-gray-500' : 'text-slate-900 dark:text-white'}`}>{variant.name}</span>
                      </div>
                      ...
                      <div className="w-full md:col-span-3">
                        {isEditing ? (
                          <div className="flex flex-col w-full">
                            <span className="text-[10px] text-gray-500 dark:text-gray-400 mb-1 font-bold">Harga OTR (Asli)</span>
                            <div className="flex items-center">
                              <span className="text-xs text-gray-500 dark:text-gray-400 mr-2">Rp</span>
                              <input
                                type="number"
                                value={editForm.originalPrice}
                                onChange={(e) => setEditForm({ ...editForm, originalPrice: Number(e.target.value) })}
                                className="w-full text-sm font-mono border border-gray-300 dark:border-slate-600 rounded px-2 py-1 outline-none focus:border-teal-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                              />
                            </div>
                          </div>
                        ) : (
                          <div className="flex justify-between md:block w-full">
                            <span className="md:hidden text-xs font-bold uppercase text-gray-400 dark:text-gray-500">Harga OTR</span>
                            <span className={`text-sm font-mono font-bold ${variant.soldOut ? 'text-gray-400 dark:text-gray-600' : 'text-gray-600 dark:text-gray-300'}`}>
                      ...
                      <div className="w-full md:col-span-3">
                        {isEditing ? (
                          <div className="flex flex-col w-full">
                            <span className="text-[10px] text-gray-500 dark:text-gray-400 mb-1 font-bold">Harga Nett (Setelah Diskon)</span>
                            <div className="flex items-center">
                              <span className="text-xs text-gray-500 dark:text-gray-400 mr-2">Rp</span>
                              <input
                                type="number"
                          ...
                          <div className="flex justify-between md:block w-full">
                            <span className="md:hidden text-xs font-bold uppercase text-gray-400 dark:text-gray-500">Harga Nett</span>
                            <div className="flex flex-col md:block text-right md:text-left">
                              <span className={`text-sm font-mono font-bold ${variant.soldOut ? 'text-gray-400 dark:text-gray-600' : 'text-teal-600 dark:text-teal-400'}`}>
                                {formatPrice(displayNett)}
                              </span>
                              {/* Show Discount Amount if applicable */}
                              {displayOTR > displayNett && (
                                <span className="text-[10px] text-green-600 bg-green-50 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded ml-2 inline-block">
                      ...
                      <div className="w-full md:col-span-2 flex justify-between md:justify-center items-center">
                        <span className="md:hidden text-xs font-bold uppercase text-gray-400 dark:text-gray-500">Status</span>
                        {isEditing ? (
                          ...
                          variant.soldOut ? (
                            <span className="px-2 py-1 bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400 text-[10px] font-bold uppercase rounded">Sold Out</span>
                          ) : (
                            <span className="px-2 py-1 bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400 text-[10px] font-bold uppercase rounded">Available</span>
                          )
                        )}
                      </div>

                      {/* Actions */}
                      <div className="w-full md:col-span-1 flex justify-end md:justify-center gap-2 border-t md:border-t-0 pt-3 md:pt-0 mt-2 md:mt-0 border-gray-100 dark:border-slate-700">
                        {isEditing ? (
                          <>
                            ...
                            <button onClick={handleCancel} className="p-2 bg-gray-200 text-gray-600 dark:bg-slate-700 dark:text-gray-400 rounded hover:bg-gray-300 dark:hover:bg-slate-600 flex items-center gap-2">
                              <X className="w-4 h-4" /> <span className="md:hidden text-xs font-bold">Cancel</span>
                            </button>
                          </>
                        ) : (
                          <button onClick={() => handleEditClick(model.id, variant)} className="p-2 text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-700 rounded transition-colors flex items-center gap-2">
                            <Edit2 className="w-4 h-4" /> <span className="md:hidden text-xs font-bold text-gray-600 dark:text-gray-400">Edit Variant</span>
                          </button>
                        )}
                      </div>
